...
use App\Models\FacultyUserDecisionVote;
...
class FacultySessionUserController extends Controller
{
    ...
    public function saveAttendance(Request $request)
    {
        ...
        foreach ($attendances as $user_id => $status) {
            ...
            $test = FacultySessionAttendanceInvite::updateOrCreate(
                ['faculty_session_id' => $session_id, 'user_id' => $user_id],
                [
    ...
    public function fetchAttendance($session_id, $locale = null)
    {
        ...
        $users = User::whereIn('id', function ($query) use ($session_id) {
            $query->select('user_id')
                ...
                ->where('faculty_session_id', $session_id)
                ->union(
        ...
        foreach ($users as $user) {
            $attendanceInvite = FacultySessionAttendanceInvite::where('user_id', $user->id)->where('faculty_session_id', $session_id)->first();
            if ($attendanceInvite) {
        ...
        $decisions = FacultySessionDecision::where('faculty_session_id', $session_id)->get();
        $decisionApproval = [];
        ...
        return view('filament.resources.faculty-session-resource.pages.attendance-list', compact('userData', 'decisionApproval', 'locale'));
    }

    public function getusersattandence($record)
    {

        $session = FacultySession::findOrFail($record);
    ...
    public function saveTime(Request $request)
    {
        ...
        $session = FacultySession::find($recordId);
    ...
    public function GetFormForSession(Request $request)
    {
        ...
        // Step 1: Get all user IDs related to the session from faculty_session_users
        $userIdsFromUsers = DB::table('faculty_session_users')
            ->where('faculty_session_id', $sessionId)
            ->pluck('user_id')
            ->toArray();

        // Step 2: Get all user IDs from faculty_session_emails
        $userIdsFromEmails = DB::table('faculty_session_emails')
            ->where('faculty_session_id', $sessionId)
            ->pluck('user_id')
            ->toArray();
        ...
        // Step 3: Get users who are absent (actual_status = 2 or 3) in SessionAttendanceInvite
        $absentUsers = FacultySessionAttendanceInvite::whereIn('actual_status', [2, 3])
            ->where('faculty_session_id', $sessionId)
        ...
        $allUsersTaken = FacultySessionAttendanceInvite::where('faculty_session_id', $sessionId)
            ->whereIn('user_id', $userIds)
            ->where('taken', '=', 1)
        ...
        // Step 4: Calculate the attendance threshold (1/3 of total users)
        $requiredAttendanceThreshold = $totalUsers / 3; // Use `ceil()` to round up

        // Step 5: Check if absent users exceed 1/3 of total users
        ...
        $topicSessionIds = FacultySessionTopic::where('faculty_session_topics.faculty_session_id', $sessionId)
        ...
        $session = FacultySession::findOrFail($sessionId);
        ...
        $sessionDecisions = FacultySessionDecision::where('faculty_session_decisions.faculty_session_id', $sessionId)
            ->whereIn('faculty_session_decisions.agenda_id', $topicSessionIds)
            ->join('topics_agendas as agendas', 'agendas.id', '=', 'faculty_session_decisions.agenda_id')
            ->join('topics', 'topics.id', '=', 'agendas.topic_id')
            ->orderBy('topics.order', 'asc') // First, order by the topic's order
            ->orderBy('faculty_session_decisions.agenda_id', 'asc') // Then, order by agenda_id
            ->select('faculty_session_decisions.*') // Explicitly select required fields
            ->get();
    ...
    {
        // dd($locale);
        return view('filament.resources.faculty-session-resource.pages.form-content', compact('locale'));
    }
        ...
        try {
            ...
            foreach ($formData as $data) {
                ...
                $decisionOrderAgenda = (int) TopicAgenda::where('id', $data['agendaId'])->where('topic_id', $data['topicId'])
                    ->value('order');
                $SessionOrderDecision = (int) FacultySession::where('id', $data['sessionId'])
                    ->value('order');
                ...
                FacultySessionDecision::updateOrCreate(
                    [
                        'agenda_id' => $data['agendaId'],
                        'topic_id' => $data['topicId'],
                        'faculty_session_id' => $data['sessionId'],
                    ],
                    [
    ...
    {
        $session = FacultySession::findOrFail($session_id);
        $decisions = FacultySessionDecision::where('faculty_session_decisions.faculty_session_id', $session_id)
            ...
            ->select('faculty_session_decisions.*', 'topics.main_topic_id') // Select fields from both tables
            ->get();
        ...
        $decisionData = $decisions->map(function ($decision) use ($topics, $session) {
            ...
            $users = User::whereIn('id', function ($query) use ($decision) {
                $query->select('user_id')
                    ->from('faculty_session_users')
                    ->where('faculty_session_id', $decision->faculty_session_id)
                    ->union(
                        DB::table('faculty_session_emails')
                            ->select('user_id')
                            ->where('faculty_session_id', $decision->faculty_session_id)
                ...
                $query->select('user_id')
                    ->from('faculty_session_attendance_invites')
                    ->where('faculty_session_id', $decision->faculty_session_id)
                    ->where('actual_status', 1);
            })->get();
            $userVotes = $users->map(function ($user) use ($decision) {
                $vote = FacultyUserDecisionVote::where('user_id', $user->id)
            ...
            return [
                ...
                'session_id' => $decision->faculty_session_id,
        ...
        return view('filament.resources.faculty-session-resource.pages.voiting-list', compact('decisionData', 'decisionApproval', 'locale'));
    }
    ...
    public function fetchVoitingSingle($session_id, $locale = null)
    {
        ...
        // Return the view with the decision data
        return view('filament.resources.faculty-session-resource.pages.voiting-list-single', compact('decisionData', 'decisionApproval', 'locale'));
    }
    ...
    public function saveVoiting(Request $request)
    {
        ...
        if ($request->voteType == 2) {
            ...
            $sessionUsers = User::whereIn('id', function ($query) use ($data) {
                $query->select('user_id')
                    ->from('faculty_session_attendance_invites')
                    ->where('faculty_session_id', $data['session_id'])
                    ->where('actual_status', 1);
            })->get();
            ...
            foreach ($data['decision'] as $decisionId => $decisionContent) {
                ...
                $decision = FacultySessionDecision::find($decisionId);
                if ($decision) {
                ...
                if (isset($data['voiting'][$decisionId])) {
                    ...
                    foreach ($votingStatuses as $userId => $voitingStatus) {
                        FacultyUserDecisionVote::updateOrCreate(
            ...
            $url = $appURL . '/admin/faculty-sessions/' . $data['session_id'] . '/start';
            ...
            foreach ($data['decision'] as $decisionId => $decisionContent) {
                ...
                if (isset($data['voiting'][$decisionId])) {
                    ...
                    // Calculate the total number of votes
                    $totalVotes = FacultyUserDecisionVote::where('decision_id', $decisionId)->count();

                    // Count the number of votes with status 1 (accept)
                    $acceptAll = FacultyUserDecisionVote::where('decision_id', $decisionId)->where('status', 1)->count();

                    // Count the number of votes with status 2 (refuse)
                    $refuseAll = FacultyUserDecisionVote::where('decision_id', $decisionId)->where('status', 2)->count();
    ...
    public function viewRecord(Request $request, $locale = null)
    {
        ...
        $sessionDecisions = FacultySessionDecision::where('faculty_session_id', $recordId)->get();
        ...
        $voteCounts = FacultyUserDecisionVote::whereIn('decision_id', $decisionIds)->count();

        // Count the number of invites for the session
        $invitesCounts = FacultySessionAttendanceInvite::where('faculty_session_id', $recordId)->where('actual_status', 1)->count();
        $countofTopic = FacultySessionTopic::where('faculty_session_id', $recordId)->count();
        ...
        return response()->json([
            ...
            'voteCount' => $voteCounts,
            'invitesCount' => $invitesCounts,
            'redirectUrl' => route('faculty-session-report', ['recordId' => $recordId]) // Add redirect URL to the response
        ]);
    }
    ...
    public function getPages($recordId)
    {
        ...
        // Build the URL dynamically
        $url = $appURL . '/admin/faculty-sessions/faculty-session-report/' . $recordId . '';
    ...
    public function decisionApproval(Request $request, $sessionId)
    {
        ...
        $unapprovedUsers = FacultySessionAttendanceInvite::where('faculty_session_id', $sessionId)
        ...
        $sessionDecisions = FacultySessionDecision::where('faculty_session_id', $sessionId)->get();
        foreach ($sessionDecisions as $decision) {
        ...
        $session = FacultySession::find($sessionId);
    ...
    public function startStopwatch(Request $request)
    {
        ...
        if ($session) {
            ...
            // Build the URL dynamically
            $url = $appURL . '/admin/faculty-sessions/' . $sessionId . '/start';

            Notification::make()
                ->title('تم بدأ جلسة مجلس الكلية')
                ->body('كلية: ' . $sessionFacultyName . ' جلسة رقم: ' . $session->code)
                ->actions([
                    NotificationsAction::make('view')
                        ->label('عرض الجلسة')
                        ->button()
    ...
    {
        $session = FacultySession::findOrFail($sessionId);
        $decisionApproval = FacultySessionDecision::where('faculty_session_id', $sessionId)->first()->approval;
        $sessionEmailsUser = FacultySessionEmail::where('faculty_session_id', $sessionId)->pluck('user_id')->toArray();
        $sessionUserIds = FacultySessionUser::where('faculty_session_id', $sessionId)->pluck('user_id')->toArray();

        $sessionUsers = array_merge($sessionUserIds, $sessionEmailsUser);


        $sessionAttendance = FacultySessionAttendanceInvite::where('faculty_session_attendance_invites.faculty_session_id', $sessionId)
            ->whereIn('user_id', $sessionUsers)
            ->join('users', 'users.id', '=', 'faculty_session_attendance_invites.user_id')
            ->join('positions', 'positions.id', '=', 'users.position_id')
            ->select(
                'users.name as userName',
                'users.signature as signature',
                'positions.ar_name as position',
                'faculty_session_attendance_invites.actual_status as attendance',
            )
            ->get();
        ...
        if ($content == "report") {
            // return view('pdf.faculty-session.report', compact('data'));
            $pdf = PDF::loadView('pdf.faculty-session.report', ['data' => $data])
            ...
            // return view('pdf.faculty-session.topics', compact('data'));
            $pdf = PDF::loadView('pdf.faculty-session.topics', ['data' => $data])
                ->setPaper('a4', 'portrait'); // Example paper size and orientation
            return $pdf->stream('Session topics (' . $newSessionCode . ')');
        } elseif ($content == "coverLetter") {
            // return view('pdf.faculty-session.agendaCoverLetter', compact('data'));
            $pdf = PDF::loadView('pdf.faculty-session.agendaCoverLetter', ['data' => $data])
                ->setPaper('a4', 'portrait'); // Example paper size and orientation
            return $pdf->stream('Session topics (' . $newSessionCode . ')');
        }
    }
    ...
    public function applySigniture(Request $request)
    {
        ...
        $attendance = FacultySessionAttendanceInvite::where('user_id', auth()->user()->id)
            ->where('faculty_session_id', $request->session_id)
            ->first();
    ...
    public function getDecisionReplacements($decision, $session, $reportTemplate, $topicCount = null)
    {
        ...
        $facultyArName = $decision->facultySessions->faculty->ar_name;
        ...
        $departmentName = TopicAgenda::where('topics_agendas.id', $decision->agenda_id)
            ->join('departments', 'departments.id', '=', 'topics_agendas.department_id')
            ->select('departments.ar_name as department_name')
        ...
        // $facultyDean = User::where('id',$decision->facultySessions->department->faculty->facultyDean->user_id)->value('name') ?? "";
        $facultyDean = User::where('faculty_id', $decision->facultySessions->faculty->id)->value('name') ?? "";
        $newDecisionNumber = $decision->facultySessions->order . '/' . $topicCount;
        ...
        $replacements = [
            ...
            '{acadimic_year}' => $decision->facultySessions->year->name ?? '',
    ...
    private function initializeMembers($session)
    {
        ...
        foreach ($users as $userId) {
            $actualStatus = FacultySessionAttendanceInvite::where('faculty_session_id', $session->id)->where('user_id', $userId)->value('actual_status');
    ...
    public function saveCollegeCouncil(Request $request, $session_id)
    {
        if (!$request->ajax()) {
            abort(404);
        }
        ...
        $data = $request->data;
        $session = Session::findOrFail($data[0]['session_id']);

        [$yearCode, $facultyCode] = explode('_', $session->code);
        $sessionOrder = 'الجلسة ' . self::sessionArabicOrdinal($session->order);
        $facultyArName = $session->department->faculty->ar_name;
        $newSessionCode = "{$yearCode}_{$facultyArName}_{$sessionOrder}";

        $sessionUsers = [$session->responsible, $session->createdBy];

        DB::beginTransaction();

        try {
            CollegeCouncil::insert($data);

            $statuses = array_column($data, 'status');
            $allAccepted = !in_array(2, $statuses);
            $allRejected = !in_array(1, $statuses);

            if ($allAccepted) {
                $this->notifySessionApproval($sessionUsers, $newSessionCode);
                $this->notifyAgendas($data, true);
            } elseif ($allRejected) {
                $this->notifySessionRejection($sessionUsers, $newSessionCode);
                $this->notifyAgendas($data, false);
            } else {
                $this->notifyIndividualAgendas($data);
            }
    ...
    private function notifySessionApproval(array $users, string $sessionCode)
    {
        Notification::make()
            ->title('تم الموافقة على جلسة القسم من العميد')
            ->body('رقم الجلسة ' . $sessionCode)
            ->sendToDatabase($users);
    }

    private function notifySessionRejection(array $users, string $sessionCode)
    {
        Notification::make()
            ->title('تم رفض جلسة القسم من العميد')
            ->body('رقم الجلسة ' . $sessionCode)
            ->sendToDatabase($users);
    }

    private function notifyAgendas(array $data, bool $approved)
    {
        foreach ($data as $item) {
            $agenda = TopicAgenda::findOrFail($item['topic_id']);
            $title = $approved ? 'تم الموافقة على طلبك من العميد' : 'تم رفض طلبك من العميد';
            $body = $approved ? null : ('سبب الرفض ' . ($item['reject_reason'] ?? ''));

            Notification::make()
                ->title($title)
                ->body($body)
                ->sendToDatabase($agenda->owner);
        }
    }

    private function notifyIndividualAgendas(array $data)
    {
        foreach ($data as $item) {
            $agenda = TopicAgenda::findOrFail($item['topic_id']);
