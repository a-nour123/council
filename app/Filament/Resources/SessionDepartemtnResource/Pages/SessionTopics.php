...
class SessionTopics extends Page
{
    protected static string $resource = SessionDepartemtnResource::class;
    protected static string $view = 'filament.resources.session-departemtn-resource.pages.session_topics';
    ...
    public $sessionResposibleId, $yearName, $topicsWithoutDecision;
    ...
    public function mount($recordId)
    {
        ...
        $this->initializeTopicsWithoutDecision($session);
    }


    protected function initializeTopicsWithoutDecision(Session $session)
    {
        ...
        // Group by main_topic
        $groupedTopics = $topicFormate->groupBy('main_topic')->map(function ($group) use ($session) {
            $formattedTopics = $group->map(function ($topic) use ($session) {
            ...
            return $formattedTopics->toArray();
        })->toArray();


        // dd($topicFormate);

        $this->topicsWithoutDecision = $groupedTopics;
    }
    ...
    protected function initializeSessionDetails(Session $session)
    {
        ...
        $this->endDateTime = Hijri::DateIndicDigits('d-m-Y', $endDateTime->format('d-m-Y'));
        ...
        return [
            Action::make('Back')
                ...
                ->action(action: function () {
                    ...
                    $url = $appURL . '/admin/session-departemtns/' . $this->recordId;
