...
class SessionReport extends Page
{
    ...
    protected static string $view = 'filament.resources.session-departemtn-resource.pages.ma7derDetails';
    ...
    public $sessionResposibleId, $departmentMessage;
    ...
    protected function initializeSessionDetails(Session $session)
    {
        ...
        $this->signitureApplyForAuthUser = SessionAttendanceInvite::where('user_id', auth()->user()->id)
        ...
        $this->absentOrNot = SessionAttendanceInvite::where('session_id', $session->id)->where('user_id', auth()->user()->id)->first()->actual_status ?? null;
    }
