...
class AttandenceList extends Page
{
    protected static string $resource = SessionDepartemtnResource::class;
    protected static string $view = 'filament.resources.session-departemtn-resource.pages.AttandenceList';
    ...
    public function mount($recordId)
    {
        ...
        $sessionInvites = SessionUser::where('session_id', $recordId)->pluck('user_id')->toArray();
        $sessionEmailsInvites = SessionEmail::where('session_id', $recordId)->pluck('user_id')->toArray();
        if (!$session) {
        ...
        $session = Session::findOrFail($recordId);
        $sessionTopic = SessionTopic::where('session_id', $recordId)->firstOrFail();
        ...
        foreach ($users as $userId) {
            $actualStatus = SessionAttendanceInvite::where('session_id', $recordId)->where('user_id', $userId)->value('actual_status');
        ...
        $decisions = SessionDecision::whereIn('session_id', [$session->id])
