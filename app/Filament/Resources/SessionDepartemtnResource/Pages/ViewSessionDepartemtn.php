...
class ViewSessionDepartemtn extends ViewRecord
{
        ...
        return [
            ...
            Action::make('Attendance')
                ...
                ->form(function ($record) {
                    ...
                    $existingRecord = SessionAttendanceInvite::where('session_id', $record->id)
                ...
                ->action(function (array $data, $record): void {
                    ...
                    $userRecord = SessionAttendanceInvite::updateOrCreate(
                        [
            ...
            Action::make('Status')
                ->visible(fn(Session $session, $record): bool => (auth()->user()->id == $session->responsible_id && $record->status != 1 && $record->status != 2))
                ->translateLabel()
                ->color('success')
                ->form(function (Session $session, $record) {
                    ...
                    $topicAgendaIds = SessionTopic::where('session_id', $record->id)->pluck('topic_agenda_id')->unique();
                ...
                ->action(function (array $data, $record): void {
                    ...
                    if ($status == 1) {
                        ...
                        $url = $appURL . '/admin/session-departemtns/' . $sessionId;

                        Notification::make()
                            ->title('لقد تمت دعوتك لحضور جلسة مجلس قسم')
                            ->body('قسم' . ': ' . $sessionDepartmentName . ' جلسة رقم: ' . $record->code)
                            ->actions([
            ...
            Action::make('Session Topics')
                ...
                ->action(action: function ($record) {
                    ...
                    // Build the URL dynamically
                    $url = $appURL . '/admin/session-departemtns/session-topics/' . $record->id;
            ...
            Action::make('Print Report')
                ...
                ->url(fn($record) => env('APP_URL') . '/admin/session-departments/details-report/' . $record->id . '/report/pdf')
                ->openUrlInNewTab(),
    ...
    private function getUserAttendanceStatus($record): string
    {
        ...
        $userRecord = SessionAttendanceInvite::where('session_id', $record->id)
            ->where('user_id', auth()->id())
            ->first();
