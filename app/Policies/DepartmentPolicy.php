...
class DepartmentPolicy
{
    ...
    {
        if (auth()->user()->hasRole('Super Admin') || auth()->user()->hasRole('System Admin') || auth()->user()->hasRole('Faculty Admin')) {
    ...
    public function update(User $user, Department $department): bool
    {
        ...
        if (auth()->user()->hasRole('Super Admin') || auth()->user()->hasRole('System Admin') || auth()->user()->hasRole('Faculty Admin')) {
            return $user->can('update_department') || $user->faculty_id === $facultyId;
    ...
    public function delete(User $user, Department $department): bool
    {
        ...
        if (auth()->user()->hasRole('Super Admin') || auth()->user()->hasRole('System Admin') || auth()->user()->hasRole('Faculty Admin')) {
            return $user->can('delete_department') || $user->faculty_id === $facultyId;
        } else {
            // return $user->can('update_department') && $user->faculty_id === $facultyId;
            return false;
        }
