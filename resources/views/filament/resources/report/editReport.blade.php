<x-filament::page>
    ...
    <div class="container mx-auto p-4">
        <form id="report-form" method="POST" action="{{ route('reports.update', ['id' => $record]) }}"
            class="bg-white p-6 rounded-lg shadow-lg border border-gray-200">
            @csrf
            @method('PUT') <!-- Correct method for updating -->
            {{-- <label for="editor" class="block text-lg font-semibold mb-2 text-gray-800">Content:</label> --}}
            ...
            <input type="hidden" value="{{ $this->existingData->topic_id }}" id="topicId" name="topicId">
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            var existingContent = `<?= addslashes($existingData['content'] ?? '') ?>`;
            quill.root.innerHTML = existingContent;
            ...
            $('#report-form').on('submit', function(event) {
                ...
                $.ajax({
                    ...
                    data: $form.serialize() +
                    '&_method=PUT', // Append _method to simulate PUT request
                    success: function(response) {
                        Swal.fire({
                    ...
                    error: function(xhr) {
                        Swal.fire({
                            title: 'Error', // Use appropriate translation here
                            text: 'Something went wrong', // Use appropriate translation here
                            icon: 'error',
                            confirmButtonText: 'OK', // Use appropriate translation here
                            confirmButtonColor: '#3085d6'
                        });
                    }
                });
