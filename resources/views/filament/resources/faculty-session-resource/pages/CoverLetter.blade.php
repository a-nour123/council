<x-filament::page>
    ...
    <div class="container">
        ...
        {{-- </button> --}}
        {{-- </div> --}}
        <h1>:السلام عليكم ورحمة الله وبركاته وبعد</h1>

        <h1>خطاب التغطية للجلسة رقم {{ $SessionCode }} لقسم {{ $depName }} لكلية {{ $facName }}</h1>
        <h1>
                    ...
                    <tr>
                        ...
                        <td class="signature-cell" style="text-align: center;">
                            ...
                            @elseif(empty($member['signature']))
                                لا يوجد توقيع
                            @else
                                                                <img src="{{ Storage::url($member['signature']) }}" alt="Signature">
                    ...
                    <tr>
                        ...
                        <td class="signature-cell" style="text-align: center;">
                            ...
                            @elseif(empty($invitedMember['signature']))
                                لا يوجد توقيع
                            @else
                                <img src="{{ Storage::url($invitedMember['signature']) }}" alt="Signature"
                                    class="signature">
                            @endif
                        </td>
                    </tr>
        ...
        <p>فقد نظر مجلس القسم فى جلسته رقم {{ $SessionCode }} لقسم {{ $depName }} لكلية {{ $facName }}
            المنعقدة يوم {{ $dayName }} {{ $higriDate }} هـ الموافق {{ $startDate }} م والتى كانت تحتوى
            على بعض القرارات لعدة مواضيع وهى كالتالى</p>
        <ol>
            @foreach ($decisions as $mainTopic => $supTopics)
                ...
                @foreach ($supTopics['details'] as $topic)
                    <h3>الموضوع {{ $this->arabicOrdinal($i) }} : {{ $topic['topic_title'] }}</h3>

                    <li>
                        {!! $topic['cover_letter_template_content'] !!}
                    </li>
                    @php $i++; @endphp
                @endforeach
            @endforeach
    ...
    <script>
        ...
        //     e.preventDefault();
        //     var sessionId = "{{ $recordId }}"; // Blade syntax for rendering session ID

        //     alert(sessionId);

        //     $.ajax({
        //         type: "GET",
        //         url: `/admin/session-departments/details-report/${sessionId}/pdf`,
        //         xhrFields: {
        //             responseType: 'blob'
        //         },
        //         success: function(response) {
        ...
        $("#pdf").click(function(e) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    // Open the PDF in a new tab for viewing and printing
